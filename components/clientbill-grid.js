	...
	Vue.component('client-grid', {
	  ...
	  template: '<table id="list-tps" class="table table-striped table-hover" style="border-collapse:collapse;">\
		...
		<template v-for="(entry, index) in paginated(\'filteredData\')">\
		     <tr data-toggle="collapse" :data-target="\'#demo\'+ index" class="accordion-toggle">\
				 <td style="padding-left: 10px;"><strong>{{entry[\'LASTNAME\']}}</strong></td>\
				 <td class="text-important"> {{entry[\'NAME\']}}</td>\
				 <td class="text-important"> {{entry[\'PHONE\']}}</td>\
				 <td class="text-important"> {{entry[\'CITY\']}}</td>\
				 <td class="text-important"> {{entry[\'POSTAL_CODE\']}}</td>\
			    </tr>\
		      <tr>\
					<td :colspan="labelColumns.length"  class="hiddenRow">\
					   <div :id="\'demo\'+index" class="accordion-body collapse"  style="padding-left: 5px; padding-top:15px;  background: #fff;border-left: 3px solid #379AFF; ">\
						 <ul class="text-muted" style="list-style: none;columns: 2;-webkit-columns: 2;-moz-columns: 2;width:100%;padding-right: 50%;padding-left: 3%;">\
						   ...
						   <a title="Quick Bill"  class="btn btn-simple btn-info table-action edit">\
					       <i class="fa fa-edit" @click="showCustomer(entry);"></i>\
					 <!--  <i class="fa fa-book" @click="showCustomer(entry);"></i>\
					       <i class="fa fa-ellipsis-h" @click="showCustomer(entry);"></i>\
					       <i class="fa fa-bar-chart" @click="showCustomer(entry);"></i>-->\
	  ...
	  methods: {
		deleteRecord: function (id) {
			exequteSQLstmnt("DELETE FROM customers WHERE ID="+ id)
		},
		showCustomer: function (customer) {
	      this.$parent.newcustomer = customer
	      showTogglePanel()
 		},  
	    sortBy: function (key) {
	      this.sortKey = key
